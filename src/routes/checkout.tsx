    ...
    return (
      <PageLayout>
        <section className="mx-auto max-w-2xl px-4 py-20 text-center">
          ...
          <Link to="/products" className="mt-6 inline-block border border-gold px-4 py-2 text-sm text-gold hover:bg-gold hover:text-black">
            Continue Shopping
          </Link>
        </section>
      </PageLayout>
        ...
        <form onSubmit={handleSubmit} className="mt-8 grid gap-8 md:grid-cols-[1fr_360px]">
            ...
            <section>
              ...
              <div className="mt-3 space-y-2">
                {[
                  ...
                  <label key={opt.v} className={`flex cursor-pointer items-start gap-3 border p-3 ${form.payment_method === opt.v ? "border-gold bg-gold/5" : "border-gold/20"}`}>
                    <input
                      type="radio"
                      name="payment"
          ...
          <aside className="h-fit border border-gold/20 bg-[#1A1A1A] p-6">
            ...
            <dl className="mt-4 space-y-2 text-sm">
              ...
              <div className="mt-3 flex justify-between border-t border-gold/10 pt-3 text-base text-white"><dt>Total</dt><dd className="font-mono text-gold">€{grandTotal.toFixed(2)}</dd></div>
            </dl>
            <button
              type="submit"
              disabled={submitting}
