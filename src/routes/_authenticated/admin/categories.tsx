...
function AdminCategoriesPage() {
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => { toast.success("Saved"); setEditing(null); qc.invalidateQueries({ queryKey: ["admin-cats"] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const del = useMutation({
    mutationFn: (id: string) => delFn({ data: { id } }),
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["admin-cats"] }); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setEditing(empty)} className="flex items-center gap-2 bg-gold px-4 py-2 text-sm font-bold text-black">
      ...
      <div className="border border-gold/15 bg-[#1A1A1A]">
        <table className="w-full text-sm">
          <thead className="text-left text-xs uppercase tracking-wider text-white/50">
            <tr><th className="p-3">Name</th><th className="p-3">Slug</th><th className="p-3">Order</th><th className="p-3"></th></tr>
          </thead>
          <tbody>
              ...
              <tr key={c.id} className="border-t border-gold/5">
                ...
                <td className="p-3">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setEditing({ id: c.id, name: c.name, slug: c.slug, description: c.description ?? "", image_url: c.image_url ?? "", sort_order: c.sort_order ?? 0 })} className="text-gold/70 hover:text-gold"><Pencil size={14} /></button>
                    <button onClick={() => confirm(`Delete ${c.name}?`) && del.mutate(c.id)} className="text-white/40 hover:text-red-400"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/80 p-4 pt-16">
          ...
          >
            <h2 className="font-serif text-lg text-white">{editing.id ? "Edit" : "New"} Category</h2>
            <Field label="Name" value={editing.name} onChange={(v) => setEditing({ ...editing, name: v, slug: editing.slug || v.toLowerCase().replace(/[^a-z0-9]+/g, "-") })} required />
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" disabled={upsert.isPending} className="bg-gold px-4 py-2 text-sm font-bold text-black disabled:opacity-50">Save</button>
            </div>
          </form>
        </div>
      )}
